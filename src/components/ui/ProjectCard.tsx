...
const accentClasses: Record<Project["accent"], { ring: string; glow: string; tag: string }> = {
  cyan: {
...
export default function ProjectCard({ project, delay = 0 }: { project: Project; delay?: number }) {
  ...
  return (
    <Reveal delay={delay} className="h-full">
      <article
        className={`group relative h-full flex flex-col rounded-2xl border border-border bg-surface/60 p-6 sm:p-7 transition-colors ${accent.ring}`}
      >
        <div
        ...
        <div className="relative flex items-start justify-between gap-4">
          <div>
            <span className={`inline-block rounded-full border px-2.5 py-0.5 text-[11px] font-mono ${accent.tag}`}>
              {project.role}
            </span>
